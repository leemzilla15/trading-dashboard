...
function scoreTradingDay({ vix, news, bias, levels, futures, chop: chopEngineResult }) {
  ...
  // Use full chopEngine result if available, otherwise fall back to simple ATR check
  const chopScore = chopEngineResult
    ? {
        score:    parseFloat((chopEngineResult.score / 10).toFixed(2)),  // 0-100 → 0-10
        max:      1.0,
        label:    chopEngineResult.label,
        detail:   chopEngineResult.action,
        isChoppy: chopEngineResult.isChoppy,
      }
    : scoreChop(levels, futures);
